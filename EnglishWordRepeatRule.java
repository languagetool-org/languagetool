...
/* LanguageTool, a natural language style checker
    ...
    USA */ package org.languagetool.rules.en;
...
/**
    ...
    Word repeat rule for English, to avoid false alarms in the generic word

    repetition rule.
    */
    public class EnglishWordRepeatRule extends WordRepeatRule {

    public EnglishWordRepeatRule(final ResourceBundle messages,
    final Language language) {
    super(messages, language);
    addExamplePair(
    Example.wrong("This is is just an example sentence."),
    ...
    * @since 2015/5/15
    ...
    if (wordRepetitionOf("that", tokens, position)&& nextPOSIsIn(tokens, position, "NN", "PRP$", "JJ", "VBZ",
    "VBD")) {
    ...
    * @since 2015/5/15
    */
    if (wordRepetitionOf("can", tokens, position)
    && POSIsIn(tokens, position, "NN")) {
    ...
    private boolean nextPOSIsIn(AnalyzedTokenReadings[] tokens, int position,
    String... posTags) {
    ...
    /**
        ...
        @return
        */
        private boolean POSIsIn(AnalyzedTokenReadings[] tokens, int position,
        String... posTags) {
        if (tokens.length > position - 1) {
        for (String posTag : posTags) {
        if (tokens[position - 1].hasPartialPosTag(posTag)) {
    ...
    /**
        ...
        @return
        */
        private boolean prePOSIsIn(AnalyzedTokenReadings[] tokens, int position,
        String... posTags) {
    ...
    private boolean wordRepetitionOf(String word,
