 ...
 *   <li>{@code prefix}: a string with prefix that is added to token.</li>
 *   <li>{@code suffix}: a string with suffix that is added to token.</li>
 * </ul>
 * @ autor Yakov Reztsov
 * Based on PartialPosTagFilter
 * @since 5.0
 */
public abstract class PrefixSuffixPosTagFilter extends RuleFilter {
  ...
  public RuleMatch acceptRuleMatch(RuleMatch match, Map<String, String> args, int patternTokenPos, AnalyzedTokenReadings[] patternTokens) {
    if (!(args.containsKey("no") && args.containsKey("regexp") && args.containsKey("postag_regexp") && args.containsKey("prefix") && args.containsKey("suffix") )) {
      throw new RuntimeException("Set 'no', 'regexp', 'postag_regexp', 'prefix' and 'suffix' for filter " + PrefixSuffixPosTagFilter.class.getSimpleName());
    }
