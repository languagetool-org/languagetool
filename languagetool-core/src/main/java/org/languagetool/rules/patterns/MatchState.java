...
public class MatchState {
	 ...
	 * @param next
	 *            Position of the next token (the skipped tokens are the ones
	...
	public final AnalyzedTokenReadings filterReadings() {
		...
		if (formattedToken != null) {
			if (match.isStaticLemma()) {
				 ...
				 * formattedToken.setWhitespaceBefore
				 * (matchedToken.isWhitespaceBefore());
				 */
			...
			if (posTag != null) {
				...
				if (match.isPostagRegexp()) {
					...
					if (l.isEmpty()) {
						for (final AnalyzedToken anaTok : getNewToken(numRead,
								token)) {
							l.add(anaTok);
						}
					}
	 ...
	 * @param sample
	 *            the sample string used to determine how the original string
