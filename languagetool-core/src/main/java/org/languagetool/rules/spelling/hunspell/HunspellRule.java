...
public class HunspellRule extends SpellingCheckRule {
  ...
  public RuleMatch[] match(AnalyzedSentence sentence) throws IOException {
    ...
    try {
      ...
      String sentenceText = getSentenceTextWithoutUrlsAndImmunizedTokens(sentence);
      String[] tokens = tokenizeText(sentenceText);
  ...
  protected String getSentenceTextWithoutUrlsAndImmunizedTokens(AnalyzedSentence sentence) {
    ...
    for (int i = 1; i < sentenceTokens.length; i++) {
      ...
      if (sentenceTokens[i].isImmunized()
        || sentenceTokens[i].isIgnoredBySpeller()
        || isUrl(token)
        || isEMail(token)
        || containsTwoOrMoreDigits(token)
        || isQuotedCompound(sentence, i, token)) {
  ...
  @NotNull
  private String buildNegativeLookaheadWithCharSet(String charSetContent) {
    return "(?![" + charSetContent + "])";
  }

  @NotNull
  private String escapeForCharSet(String string) {
