...
public class SuggestionExtractorTest extends TestCase {
  ...
  public void testGetSuggestions() throws IOException {
    ...
    assertEquals("[foo bla, xxx]", extractor.getSimpleSuggestions("Did you mean <suggestion suppress_misspelled=\"yes\">foo bla</suggestion>" +
            " or <suggestion>xxx</suggestion>?").toString());

    assertEquals("[]", extractor.getSimpleSuggestions("Did you mean <suggestion>foo \\1</suggestion>?").toString());
    assertEquals("[]", extractor.getSimpleSuggestions("Did you mean <suggestion>‚<match no=\"3\" include_skipped=\"following\"/></suggestion>?").toString());
  }
