...
public class RegexExpander {
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (String line : lines) {
      ...
      if (StringTools.startsWithLowercase(line) && p.matcher(StringTools.uppercaseFirstChar(line)).matches()) {
        ucMatch = true;
      }
      if (lcMatch && ucMatch) {
        printToken(i, "[" + StringTools.uppercaseFirstChar(line).charAt(0) + StringTools.lowercaseFirstChar(line).charAt(0) + "]" + line.substring(1));
        i++;
      } else if (lcMatch) {
        printToken(i, line);
        i++;
      } else if (ucMatch) {
        printToken(i, StringTools.uppercaseFirstChar(line));
        i++;
      }
    }
  }

  private static void printToken(int i, String line) {
    if (i == 0) {
