 ...
 * (0 = no error, 1 = error). For example, these inputs
 * 
...
class TrainingDataGenerator {
    ...
    for (ErrorSentence sentence : corpus) {
      ...
      for (Error error : errors) {
        try {
          ...
          writeTrainingData(errorContext);
          List<String> correctedContext = get2gramContext(sentence, error, true);
          writeTrainingData(correctedContext);
        } catch (AmbiguousErrorPositionException e) {
          System.err.println("Ignoring sentence due to limited algorithm: '" + sentence + "'");
          problemCount++;
        }
      }
    }
  ...
  private List<String> get2gramContext(ErrorSentence sentence, Error error, boolean useCorrection) {
    ...
    String errorOrCorrection = useCorrection ? error.getCorrection() : markupText.substring(error.getStartPos(), error.getEndPos());
    List<Integer> errorTokens = new ArrayList<>();
    int i = 0;
    for (String token : tokens) {
      if (token.equals(errorOrCorrection)) {
        errorTokens.add(i);
      }
      i++;
    }
    ...
    if (errorTokenPos == 0) {
      ...
      contextTokens.add(tokens.get(errorTokenPos));
  ...
  private void writeTrainingData(List<String> context) {
    ...
    System.out.println(leftCount + "\t" + rightCount + "\t" + "01?" + "\t" + context);  // TODO: 0 or 1
  }
