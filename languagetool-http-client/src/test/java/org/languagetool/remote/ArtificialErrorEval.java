...
public class ArtificialErrorEval {
  ...
  public static void main(String[] args) throws IOException {
    ...
    final Pattern p0 = Pattern.compile("\\b" + words[0] + "\\b", Pattern.CASE_INSENSITIVE);
    final Pattern p1 = Pattern.compile("\\b" + words[1] + "\\b", Pattern.CASE_INSENSITIVE);
    int count = 0;
      ...
      if (match.getErrorOffset() <= pos && match.getErrorOffset() + match.getErrorLength() > pos) {
        for (String s: match.getReplacements().get()) {
          ...
          String correctedSentence = wrongSentence.substring(0, match.getErrorOffset()) + s
              + wrongSentence.substring(match.getErrorOffset() + s.length(), wrongSentence.length());
