 ...
 * @since 2.3
 */
public class ArabicChunker implements Chunker {

  private static final String TOKENIZER_MODEL = "/ar-token.bin";
  private static final String POS_TAGGER_MODEL = "/ar-pos-maxent.bin";
  private static final String CHUNKER_MODEL = "/ar-chunker.bin";

  /**
   * This needs to be static to save memory: as Language.LANGUAGES is static, any language
   * that is once created there will never be released. As Arabic has several variants,
  ...
  private final ArabicChunkFilter chunkFilter;

  public ArabicChunker() {
    try {
      if (tokenModel == null) {
      ...
      chunkFilter = new ArabicChunkFilter();
    } catch (IOException e) {
      throw new RuntimeException("Could not initialize Arabic chunker", e);
    }
  }
