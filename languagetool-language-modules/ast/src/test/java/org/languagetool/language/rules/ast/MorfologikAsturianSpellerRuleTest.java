...
public class MorfologikAsturianSpellerRuleTest {
  ...
  public void testMorfologikSpeller() throws IOException {
    ...
    assertEquals(0, rule.match(lt.getAnalyzedSentence("¿Afáyeste colos correutores gramaticales?")).length);

    RuleMatch[] matches = rule.match(lt.getAnalyzedSentence("¿Afáyiste colos correutores gramaticales?"));
    assertEquals(1, matches.length);
    assertEquals(1, matches[0].getFromPos());
    assertEquals(9, matches[0].getToPos());
    assertEquals("Afáyeste", matches[0].getSuggestedReplacements().get(0));
  }
