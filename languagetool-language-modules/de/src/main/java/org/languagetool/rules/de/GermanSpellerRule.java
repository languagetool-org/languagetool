...
public class GermanSpellerRule extends CompoundAwareHunspellRule {
  ...
  private static final Pattern PERSON_SUFFIXES = compile(".+([dt]ant(in)?|graf(in)?|([bdfghnoptuvxyz]|he|([in]a|o)(l)|(for|ga|i)m|[^f]r)(ist)(in)?|krat(in)?|man(e|in)|olog(e|in)|[pt]ient(in)?|soph(in)?|.+[^b]urg(in)?)$");
  ...
  protected List<SuggestedReplacement> getOnlySuggestions(String word) {
    if (SYMPHATISCH.matcher(word).matches()) {
      return topMatch(word.replaceFirst("symphatisch", "sympathisch"));
    }
