...
public class GreekRedundancyRuleTest {
	...
	public void testRule() throws IOException {
		Assertions.assertEquals(0, rule.match(langTool.getAnalyzedSentence("Τώρα μπαίνω στο σπίτι.")).length);
		Assertions.assertEquals(0, rule.match(langTool.getAnalyzedSentence("Απόψε θα βγω.")).length);
	}
	...
	public void testRuleWithinSentence() throws IOException {
		...
		Assertions.assertEquals("μπαίνω", matches[0].getSuggestedReplacements().get(0));
	}
	...
	public void testRuleBegginingOfSentence() throws IOException {
		...
		Assertions.assertEquals(1, matches.length);
		Assertions.assertEquals("Απόψε", matches[0].getSuggestedReplacements().get(0));
	}
	...
	public void testRuleMultipleSuggestions() throws IOException {
		...
		Assertions.assertEquals(1, matches.length);
		Assertions.assertEquals("αλλά,όμως", matches[0].getSuggestedReplacements().get(0));
