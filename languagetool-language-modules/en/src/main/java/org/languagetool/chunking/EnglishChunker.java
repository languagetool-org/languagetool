...
public class EnglishChunker implements Chunker {
  ...
  public void addChunkTags(List<AnalyzedTokenReadings> tokenReadings) {
    List<ComparableChunkTaggedToken> origChunkTags = getChunkTagsForReadings(tokenReadings);
    List<ComparableChunkTaggedToken> chunkTags = chunkFilter.filter(origChunkTags);
    assignChunksToReadings(chunkTags);
  }

  private List<ComparableChunkTaggedToken> getChunkTagsForReadings(List<AnalyzedTokenReadings> tokenReadings) {
  ...
  private List<ComparableChunkTaggedToken> getTokensWithTokenReadings(List<AnalyzedTokenReadings> tokenReadings, String[] tokens, String[] chunkTags) {
    ...
    for (String chunkTag : chunkTags) {
      ...
      ArrayList<ChunkTag> l = new ArrayList<>();
      l.add(new ChunkTag(chunkTag));
      result.add(new ComparableChunkTaggedToken(tokens[i], l, readings, startPos));
      pos = endPos;
      i++;
    }
    return result;
  ...
  private void assignChunksToReadings(List<ComparableChunkTaggedToken> ComparableChunkTaggedTokens) {
    for (ComparableChunkTaggedToken taggedToken : ComparableChunkTaggedTokens) {
