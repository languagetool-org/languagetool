...
public class EnglishWordRepeatBeginningRuleTest {
  ...
  public void testRule() throws IOException {
    ...
    Assertions.assertEquals(0, lt.check("This is good. This is good, too.").size());
    // three successive sentences that start with the same exception word ("the").
    Assertions.assertEquals(0, lt.check("The car. The bicycle. The third sentence with 'the'.").size());
    ...
    Assertions.assertTrue(matches3.get(0).getSuggestedReplacements().stream().anyMatch(sugg ->  sugg.equals("Additionally")));
    Assertions.assertTrue(matches3.get(0).getSuggestedReplacements().stream().anyMatch(sugg ->  sugg.equals("Besides")));
    Assertions.assertTrue(matches3.get(0).getSuggestedReplacements().stream().anyMatch(sugg ->  sugg.equals("Furthermore")));
    Assertions.assertTrue(matches3.get(0).getSuggestedReplacements().stream().anyMatch(sugg ->  sugg.equals("Moreover")));
    Assertions.assertTrue(matches3.get(0).getSuggestedReplacements().stream().anyMatch(sugg ->  sugg.equals("In addition")));
    Assertions.assertTrue(matches3.get(0).getSuggestedReplacements().stream().anyMatch(sugg ->  sugg.equals("As well as")));
  }
