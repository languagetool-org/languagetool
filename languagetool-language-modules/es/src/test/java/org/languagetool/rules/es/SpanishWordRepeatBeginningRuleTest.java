...
public class SpanishWordRepeatBeginningRuleTest {
  ...
  public void testRule() throws IOException {
    ...
    Assertions.assertEquals(0, lt.check("Esto está bien. Esto es mejor, también.").size());
    // three successive sentences that start with the same exception word ("the").
    Assertions.assertEquals(0, lt.check("El coche. El profesor. El tercer elemento.").size());

    // =================== errors =============================
    Assertions.assertEquals(2, lt.check("Mañana me voy. Mañana vuelvo. Mañana se acabó todo.").size());
