...
# Simplification
sed -e '1,16 d; s/\([^\t]*\)[\t]\([^\t]*\)[\t]\([^\t]*\)[\t]\([^\t]*\)[\t]\([^\t]*\)[\t]\([^\t]*\)[\t]\([^\t]*\)[\t]\(.*\)\([^\t]*\)[\t]\(.*\)/\2\t\3\t\4\t/g; s/[\t]mg /\t/g; s/prep prepv/prep/g; s/ preverb//g; s/adv negadv/adv/g; s/negadv/adv/g; s/ detex//g; s/ proadv//g; s/\(.*\)\(prep\)\(.*\)\( loc\.prep\)\(.*\)/\1\2\3\5/g; s/loc\.nom/nom/g;' $Input.txt  > $Input.01.txt
