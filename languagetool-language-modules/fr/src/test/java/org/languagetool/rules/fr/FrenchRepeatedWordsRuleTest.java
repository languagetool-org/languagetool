...
public class FrenchRepeatedWordsRuleTest {
  ...
  public void testRule() throws IOException {
    ...
    Assertions.assertEquals("[particulièrement, spécialement, singulièrement, surtout, spécifiquement]", matches[0].getSuggestedReplacements().toString());
    
    matches = getRuleMatches("Elle est maintenant phénoménale. Les choses sont maintenant compliquées.");
    Assertions.assertEquals(1, matches.length);
    Assertions.assertEquals("[présentement, ce jour-ci, désormais, à présent]", matches[0].getSuggestedReplacements().toString());
