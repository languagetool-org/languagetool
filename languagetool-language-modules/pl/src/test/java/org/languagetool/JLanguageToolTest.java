...
public class JLanguageToolTest {
  ...
  public void testPolish() throws IOException {
    ...
    Assertions.assertEquals("[PL]", Arrays.toString(polish.getCountries()));
    ...
    Assertions.assertEquals(0, noRulesTool.check("Mają one niemałe znaczenie.").size());
    Assertions.assertEquals(0, noRulesTool.check("Często wystarczy obrócić na wspak wyroki świata, aby trafnie osądzić jakąś osobę.").size());
    //with immunization
    Assertions.assertEquals(0, noRulesTool.check("A teraz każcie mi dać jaki bądź posiłek.").size());
    Assertions.assertEquals(0, noRulesTool.check("Kiedym wóz zobaczył, byłbym przysiągł, że wielka przygoda mnie czeka.").size());
    //with antipatterns: "wymaluj" in "wypisz wymaluj" is immunized locally for punctuation mistakes,
    //so it should get no match
    Assertions.assertEquals(0, noRulesTool.check("Jurek wygląda wypisz wymaluj babcia.").size());
    //but it should get a match with word repetitions:
    Assertions.assertEquals(1, noRulesTool.check("Jurek wygląda wypisz wypisz wymaluj babcia.").size());
    Assertions.assertEquals(1, noRulesTool.check("Jurek wygląda wypisz wymaluj wymaluj babcia.").size());
    //check for a weird unification bug:
    Assertions.assertEquals(0, noRulesTool.check("Zawarł w niej, oprócz swojej twórczości, wybrane epigramaty czterdziestu ośmiu innych greckich poetów i poetek.").size());
    //checking on pattern rules now...
    //now this should be immunized:
    Assertions.assertEquals(0, tool.check("Nudne brednie tak zamąciły głowę chłopu, że klął na czym ziemia stoi, zmuszonym będąc słuchać tego wszystkiego.").size());
    //but this "chcąc, nie chcąc" immunized only by an antipattern
    Assertions.assertEquals(1, tool.check("Chcąc, nie chcąc zjadłem pstrąga.").size());
    ...
    Assertions.assertEquals("[., Polish, This, is, text]", tool.getUnknownWords().toString());
    //check positions relative to sentence ends
    matches = tool.check("To jest tekst.\nTest 1. To jest linia w której nie ma przecinka.");
    Assertions.assertEquals(17, matches.get(0).getColumn());
    //with a space...
    matches = tool.check("To jest tekst. \nTest 1. To jest linia w której nie ma przecinka.");
    Assertions.assertEquals(16, matches.get(0).getColumn());
    matches = tool.check("To jest tekst. Test 1. To jest linia w której nie ma przecinka.");
    Assertions.assertEquals(32, matches.get(0).getColumn());
