...
public class PortugueseWordRepeatBeginningRuleTest {
  ...
  public void testRule() throws IOException {
    ...
    assertEquals(0, langTool.check("2011: Setembro já passou. 2011: Outubro também já passou. 2011: Novembro já se foi.").size());
     assertEquals(1, langTool.check("Certo, isto está bem. Este exemplo está correto. Certo que este também.").size()); // 1 error from NO_VERB
    // errors:
    assertEquals(2, langTool.check("Este exemplo está correto. Este segundo também. Este terceiro exemplo não.").size()); //1 error from NO_VERB
