...
public class PortugueseWordRepeatBeginningRuleTest {
  ...
  public void testRule() throws IOException {
    ...
    assertEquals(0, langTool.check("Certo que sim. Este exemplo está correcto. Certo que este também.").size());
    assertEquals(0, langTool.check("2011: Setembro já passou. 2011: Outubro também. 2011: Novembro já se foi.").size());
    // errors:
    assertEquals(1, langTool.check("Este exemplo está correto. Este terceiro exemplo não.").size());
