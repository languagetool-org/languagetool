...
public class RomanianSynthesizerTest {
  ...
  public final void testSynthesizeStringString() throws IOException {
    ...
    // a alege
    // forma de infinitiv
    Assertions.assertEquals("[alege]", Arrays.toString(synth.synthesize(
        dummyToken("alege"), "V000000f00")));
    // conjunctiv, pers a doua plural
    Assertions.assertEquals("[alegeți]", Arrays.toString(synth.synthesize(
        dummyToken("alege"), "V0p2000cz0")));

    // a fi
    Assertions.assertEquals("[fi]", Arrays.toString(synth.synthesize(
        dummyToken("fi"), "V000000f0f")));
    // indicativ prezent, pers a doua plural
    Assertions.assertEquals("[sunteți]", Arrays.toString(synth.synthesize(
        dummyToken("fi"), "V0p2000izf")));
    // indicativ prezent, pers a treia plural
    Assertions.assertEquals("[sunt]", Arrays.toString(synth.synthesize(
    ...
    Assertions.assertEquals("[sunteți, sunt]", Arrays.toString(synth.synthesize(
        dummyToken("fi"), "V0p2000izf|V0p3000izf", true)));

    // diverse
    // indicativ, mai mult ca perfect, persoana întâi, plural
    Assertions.assertEquals("[merseserăm]", Arrays.toString(synth.synthesize(
        dummyToken("merge"), "V0p1000im0")));
    // indicativ, mai mult ca perfect, persoana întâi, singular
    Assertions.assertEquals("[mersesem]", Arrays.toString(synth.synthesize(
        dummyToken("merge"), "V0s1000im0")));
    Assertions.assertEquals("[legătura]", Arrays.toString(synth.synthesize(
        dummyToken("legătură"), "Sfs3aac000")));
    Assertions.assertEquals("[legătură]", Arrays.toString(synth.synthesize(
        dummyToken("legătură"), "Sfs3anc000")));

    // user data (/ro/added.txt)
    Assertions.assertEquals("[configurați]", Arrays.toString(synth.synthesize(
        dummyToken("configura"), "V0p2000cz0"))); // no reg exp
    Assertions.assertEquals("[configurați, configurezi]", Arrays.toString(synth.synthesize(
