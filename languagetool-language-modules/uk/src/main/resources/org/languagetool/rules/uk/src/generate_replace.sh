...
grep -h " #>" $spell_uk_dir/src/Dictionary/twisters.lst | sed -r "s/^([а-яіїєґ'-]+).*#> *(.*)/\1=\2/i" >> $REPLACE_FILE
...
ls -1 $spell_uk_dir/src/Dictionary/*.lst | grep -v twisters | xargs cat | grep -h " #>" | sed -r "s/^([а-яіїєґ'-]+).*#> *(.*)/\1=\2/i; s/ *[;,] */|/g" | sort >> $REPLACE_FILE_2
