...
#,EUPHONY_PREP_V_U
RULES_TO_IGNORE="MORFOLOGIK_RULE_UK_UA,COMMA_PARENTHESIS_WHITESPACE,WHITESPACE_RULE,UK_MIXED_ALPHABETS,UK_SIMPLE_REPLACE,UK_SIMPLE_REPLACE_SOFT,EUPHONY_OTHER,EUPHONY_PREP_V_U,INVALID_DATE,YEAR_20001,DATE_WEEKDAY1,DASH,UK_HIDDEN_CHARS,UPPER_INDEX_FOR_M,DEGREES_FOR_C,OVKA_FOR_PROCESS"
...
function run_lt() 
{
  ...
  ID="$2"
    java $JAVA_OPTS -cp $BASE1:$BASE2:$BASE3:$BASE4:$CPATH -Dorg.languagetool.tagging.uk.UkrainianTagger.debugCompounds=false \
       org.languagetool.commandline.Main -l uk -d $RULES_TO_IGNORE$EXTRA $SRC | \
