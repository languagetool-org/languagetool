...
cat all.srt.tmp | grep " anim-" | awk '{print $1}' | uniq > compounds-unknown-anim-inanim.txt
cat all.srt.tmp | grep " inanim-" | awk '{print $1}' | uniq > compounds-unknown-inanim-anim.txt
rm all.srt.tmp
