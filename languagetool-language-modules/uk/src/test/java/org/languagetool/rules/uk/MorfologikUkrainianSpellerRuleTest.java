...
public class MorfologikUkrainianSpellerRuleTest {
  ...
  public void testMorfologikSpeller() throws IOException {
    // correct sentences:
    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("До вас прийде заввідділу!")).length);
    ...
    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("До нас приїде The Beatles!")).length);

    // soft hyphen
    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("піс\u00ADні")).length);
    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("піс\u00ADні піс\u00ADні")).length);

    // non-breaking hyphen
    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("ось\u2011ось")).length);

    // frequent infix notation
    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("-ськ-")).length);

    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Халгін-Гол")).length);
...
//    assertEquals("склянка", matches[0].getSuggestedReplacements().get(0));
    ...
    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("а")).length);

    // mix alphabets
    matches = rule.match(lt.getAnalyzedSentence("прийдешнiй"));   // latin 'i'

    Assertions.assertEquals(1, matches.length);
    Assertions.assertEquals("прийдешній", matches[0].getSuggestedReplacements().get(0));
...
//    assertEquals(0, matches.length);
    ...
    Assertions.assertEquals(new ArrayList<RuleMatch>(), Arrays.asList(match));
    ...
    match = rule.match(lt.getAnalyzedSentence("100 кв м"));
    Assertions.assertEquals(1, Arrays.asList(match).size());
...
//    assertEquals(1, Arrays.asList(match).size());
    ...
    Assertions.assertEquals(0, match.length);
    ...
    Assertions.assertEquals(1, Arrays.asList(match).size());
    Assertions.assertEquals(sent.indexOf("Петро"), match[0].getFromPos());
    Assertions.assertEquals(sent.length(), match[0].getToPos());

    sent = "голага́нівська";
    match = rule.match(lt.getAnalyzedSentence(sent));
    Assertions.assertEquals(1, Arrays.asList(match).size());
    Assertions.assertEquals(0, match[0].getFromPos());
    Assertions.assertEquals(sent.length(), match[0].getToPos());
...
//    assertEquals(sent.length(), match[0].getToPos());
  ...
  public void testSuggestionOrder() throws IOException {
    ...
    Assertions.assertEquals(1, Arrays.asList(match).size());
    Assertions.assertEquals(Arrays.asList("захворів", "захворіла", "захворіє", "захворівши", "захворював"), match[0].getSuggestedReplacements());
  }
  ...
  public void testCompounds() throws IOException {
    ...
    // compounding
    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("Жакет був синьо-жовтого кольору")).length);

    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("Він багато сидів на інтернет-форумах")).length);

    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Він багато сидів на інтермет-форумах")).length);

    
    // dynamic tagging
    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("екс-креветка")).length);

    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("банд-формування.")).length);

//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("учбово-виховного")).length);

    Assertions.assertEquals(0, rule.match(lt.getAnalyzedSentence("екоблогер")).length);
  }
  ...
  public void testDashedSuggestions() throws IOException {
    ...
    Assertions.assertEquals(1, match.length);
    Assertions.assertEquals(Arrays.asList("блок система", "блок-система"), match[0].getSuggestedReplacements());
...
//    assertEquals(Arrays.asList("онлайн-навчання"), match[0].getSuggestedReplacements());
    ...
    Assertions.assertTrue(match[0].getSuggestedReplacements().isEmpty(), "Should be empty: " + match[0].getSuggestedReplacements());
...
//    assertEquals(1, match.length);
    ...
    Assertions.assertTrue(match[0].getSuggestedReplacements().isEmpty(), "Unexpected suggestions: " + match[0].getSuggestedReplacements());
