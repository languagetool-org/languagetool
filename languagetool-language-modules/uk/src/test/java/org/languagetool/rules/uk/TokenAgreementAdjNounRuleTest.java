...
//    TokenInflectionAgreementRule.DEBUG = true;
  ...
  public void testRule() throws IOException {
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("скрутна справі")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("район імпозантних віл")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("офіційний статистика")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("зелена яблуко.")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("сп’янілі свободою")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("кволий депутата")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("кволого тюльпан")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("цинічна винахідливості")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("наступній рік свого життя")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("жодного кубічного метру в Україні не буде")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("складний рік на фондовим ринку")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("є найкращий засобом для очистки")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("має вчену ступінь з хімії")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("розкішні чорні коси й, засукавши широкі рукава своєї сорочки,")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("то коло стола її вишивані рукава мають, то коло печі")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("валялись одірвані рукава кунтушів та поли жупанів")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("вчинили страшенний диплома тичний галас")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Раймон Бенжамен і керівник європейського філії")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("обвинувачення у вчинені злочину, передбаченого")).length);

    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("перша ступінь")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("друга ступінь")).length);
...
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("французька політик")).length);
    ...
    Assertions.assertEquals(1, matches0.length);
    Assertions.assertTrue(matches0[0].getMessage().contains("[ч.р.: родовий, знахідний]"), "Message is wrong: " + matches0[0].getMessage());
    Assertions.assertEquals(Arrays.asList("російських винищувачів", "російського винищувача"), matches0[0].getSuggestedReplacements());
...
//    assertEmptyMatch("З усіх опитаних майже половина відверто");
    ...
    // і-и
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("під зеківській нуль")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("у повітряній простір")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("в дитячий лікарні")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("у Київський філармонії")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("на керівні посаді")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("незворотній процес")).length);
    // taken care by xml rule
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("408 зниклих безвісті")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("нинішній російські владі")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("заробітної платі")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("сталеві панцирі")).length);
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("бейсбольною битою машини")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("У львівській та київський Книгарнях")).length);
    // relies on disambiguation
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("— робочій день.")).length);
    assertEmptyMatch("президентів Леонідів Кравчука та Кучму");

    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("угода на 30 років зі щорічного поставкою росіянами ...")).length);
    
    // missing/extra letter
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("сприймали так власні громадян")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("прокатні транспорті засоби")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Ви не притягнене капіталу")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("потрібна змін поколінь")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Незадовільне забезпеченням паливом")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("сочиться коштовний камінням")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("будь-якої демократичної крани")).length);
    // case government
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("що найбільший досягненням")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Генеральній прокураторі")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("була низька передумов")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("після смерті легендарного Фреді Меркюрі")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("юна викрадача й не здогадувалася")).length);

    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("По повернені кореспондентів")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("в очікувані експериментатора")).length);

    // wrong letter
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("певної мірою")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("люмпенізується дедалі більша частини")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Державна фіскальну служба")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Як правило, це чоловіки, годувальними сімей")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("з московською боку")).length);

//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("від войовничих хозар")).length);

    // wrong gender
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("з насиджених барліг")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("У польському Лодзі")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("панування зимових сутінок")).length);
    
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("за наявною інформацію")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("асоціюється в нас із сучасною цивілізацію")).length);
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("які вимагалися за тендерною документацію")).length);

    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("зловживання монопольних становищем")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("проживання та дворазове харчуванням")).length);
...
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("паралельно приймаючі пацієнтів")).length);
    ...
    Assertions.assertTrue(matches[0].getMessage().contains("Можливо"), "Missing message for v_rod/v_dav -у/ю");

    matches = rule.match(lt.getAnalyzedSentence("Та пахових ділянках"));
    Assertions.assertEquals(1, matches.length);
    Assertions.assertTrue(matches[0].getMessage().contains("Можливо"), "Missing message for v_mis");

    // false v_rod with -а
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("федерального округа")).length);
...
//    assertTrue("Missing message for «не» пишеться окремо", matches[0].getMessage().contains("писати окремо"));
    ...
    // false :nv
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("затверджений народним віче")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("На великому родинному віче")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("в листопаді 2015 року на народні віче до Кривого Рогу")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("як японські ніндзя")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("приталені пальто")).length);
    
    // missing/extra space, dash etc
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("соціальними мережа ми")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("принциповими країна ми")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("отримала мандатна ведення")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("по вуличному Копійчина")).length);

    // lowercase city
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("У мінську влада")).length);
...
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("двометрові забори")).length);
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("вздовж дніпровської вісі")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("ніщо так не зближає приморських партизан")).length);
...
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("що робить її найвищою будівля")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("як боротьбу сунітської більшість")).length);
    
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("найцікавішій час")).length);
  }
...
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("Та схоже суд таки вийшов")).length);
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("від наступних пари")).length);
...
//    assertEmptyMatch("тепер хоч по Підвальній трамваї можуть пройти");
  ...
  public void testExceptionsNumbers() throws IOException {
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("два високих депутат")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("дві високих дівчині")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("83,7 квадратних кілометра")).length);
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("п'ять шості світу")).length);

    assertEmptyMatch("1/8-ї фіналу");
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("1/8-ї фіналом")).length);
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("дві других дівчині")).length);
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("3-й класи поснідали")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("одному-два непоганих шанси")).length);
    
    RuleMatch[] match = rule.match(lt.getAnalyzedSentence("залишилося сиротами 22-є дітей"));
    Assertions.assertEquals(1, match.length);
    Assertions.assertTrue(match[0].getMessage().contains("літерне нарощення після кількісного числівника"));
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("на 131-му хвилині")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("О 12–ї годині")).length);
  ...
  public void testExceptionsOther() throws IOException {
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("у Великій Вітчизняній Війн")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("у Великій Вітчизняній війна")).length);
...
//    assertEmptyMatch("бачити на Різницький представника донецького регіону");
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("на Західній України")).length);
  ...
  public void testExceptionsAdjp() throws IOException {
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("помальований в біле кімната")).length);
...
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("вкриті плющем будинок")).length);
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("підсвічений синім діамантів")).length);
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Нав’язаний Австрії нейтралітеті")).length);
    //TODO:
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("змучений тягарем життю")).length);
  ...
  public void testExceptionsVerb() throws IOException {
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("слід бути обережною туристам у горах")).length);
...
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("партія вважає доцільним участь у списку осіб")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("вважають нелегітимними анексію")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("не залишили байдужими адміністрацію")).length);
...
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("визнають регіональною облради")).length);
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("визнання неконституційними закону")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("визнання недійсним рішенню")).length);
...
//    assertEmptyMatch("cюди Більше порад");
  ...
  public void testExceptionsAdj() throws IOException {
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("жадібна землею")).length);
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("альтернативну олігархічній порядку")).length);
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("альтернативну олігархічному модель")).length);
  ...
  public void testExceptionsPrepAdj() throws IOException {
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("він є одним із найстаріший амфітеатрів")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("подібний до попереднього закони")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("порівняний з попереднім результатів")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("Схожої з тамтешньою концепція")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("протилежний до загальнодержавному процес")).length);
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("вдалися до збройної боротьбі")).length);
...
//  assertEmptyMatch("її острови відрізняються від Південної природою");
  ...
  public void testExceptionsPlural() throws IOException {
    ...
    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("на довгих місяця")).length);

    Assertions.assertEquals(1, rule.match(lt.getAnalyzedSentence("продукту, а просунути український фільми")).length);
...
//    assertEquals(1, rule.match(langTool.getAnalyzedSentence("на вибори само висуванцем")).length);
  ...
  private void assertEmptyMatch(String text) {
    try {
      Assertions.assertEquals(Collections.<RuleMatch>emptyList(), Arrays.asList(rule.match(lt.getAnalyzedSentence(text))));
    } catch (IOException e) {
  ...
  private void assertHasError(String text) {
    try {
      ...
      Assertions.assertEquals(1, rule.match(sent).length);
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  }
