 ...
 * @author Fred Kruse
 */
public class DocumentTextCache implements Serializable {
  ...
  DocumentTextCache(DocumentType docType) {
    debugMode = OfficeTools.DEBUG_MODE_DC;
    debugModeTm = OfficeTools.DEBUG_MODE_TM;
    this.docType = docType;
  }

  DocumentTextCache(SingleDocument document, Locale fixedLocale, Locale docLocale,
  ...
  DocumentTextCache(DocumentTextCache in) {
...
*/
   ...
   */
  private void add(DocumentTextCache in) {
   ...
   */
  public void put(DocumentTextCache in) {
    rwLock.writeLock().lock();
    try {
      clear();
      add(in);
...
*/  
  ...
  public ChangedRange refreshAndCompare(SingleDocument document, Locale fixedLocale, Locale docLocale, XComponent xComponent, int fromWhere) {
    DocumentTextCache oldCache = new DocumentTextCache(this);
    ...
    try {
      ...
      if (oldCache.toParaMapping.get(CURSOR_TYPE_HEADER_FOOTER).size() != toParaMapping.get(CURSOR_TYPE_HEADER_FOOTER).size()) {
        while (from < paragraphs.size() && from < oldCache.paragraphs.size()
            && (toTextMapping.get(from).type != DocumentTextCache.CURSOR_TYPE_HEADER_FOOTER
            || paragraphs.get(from).equals(oldCache.paragraphs.get(from)))) {
          from++;
        }
        while (to <= paragraphs.size() && to <= oldCache.paragraphs.size()
            && (toTextMapping.get(paragraphs.size() - to).type != DocumentTextCache.CURSOR_TYPE_HEADER_FOOTER
