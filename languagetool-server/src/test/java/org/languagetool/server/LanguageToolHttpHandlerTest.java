...
public class LanguageToolHttpHandlerTest {

  private final LinkedBlockingQueue<Runnable> workQueue = new LinkedBlockingQueue<>();
  private final LanguageToolHttpHandler handler = new LanguageToolHttpHandler(false, Collections.emptySet(), false, null, workQueue);
  ...
  public void testInvalidPreferredVariant() {
    handler.detectLanguageOfString(english, "de", Arrays.asList("en"));  // that's not a variant
  }

  @Test(expected = RuntimeException.class)
  public void testInvalidPreferredVariant2() {
    handler.detectLanguageOfString(english, "de", Arrays.asList("en-YY"));  // variant doesn't exist
  }
