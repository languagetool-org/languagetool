...
for PROJECT in languagetool-core en fr de pl ca it br nl pt ru ast be zh da eo gl el is ja km lt ml ro sk sl es sv tl uk fa ta de-DE-x-simple-language all languagetool-gui-commons languagetool-wikipedia languagetool-server languagetool-http-client
do
    ...
    if [ $PROJECT != "all" -a $PROJECT != "hunspell-native-libs" ]
    then
        if [ ! -f $JAVADOC_FILE ]
        then
