...
/**
 * Command line tool to extract expressions, that their words are starting with
 * capital letter, from a (optionally bz2-compressed) Wikipedia XML dump.
 */
class WikipediaSpecificCaseExpressionExtractor {
	 ...
	 * they are found
	 */
	private static HashMap<String, Integer> specificCaseExpressionsCounter = new HashMap<String, Integer>();
	 ...
	 */
	private void extractSpecificCaseExpressions(Language language, String xmlDumpPath, String outputFile)
	 ...
	 * specifcCaseExpressionCounter.
	 * 
	 * @param sentence the sentence to detect specific case ecptessions
	 */
	private void detectSpecificCaseExpressions(String sentence) {
		Queue<String> specificCaseQueue = new LinkedList<>(); // contains back to back words that start with uppercase
		...
		for (int i = 1; i < words.length; i++) { // starting from 1 as the first word of the sentence will always start
													// with capital
			String word = words[i];
			...
			word = word.replaceAll("^[^α-ωΑ-Ωίϊΐόάέύϋΰήώ'\\s]+|[^α-ωΑ-Ωίϊΐόάέύϋΰήώ'\\s]+$", ""); // remove any special
																									// characters like
																									// brackets and
																									// commas except " '
																									// "
	...
	private void incrementSpecificCaseExpressionsCounter(Queue<String> queueContainingExpression) {
		...
		int count = specificCaseExpressionsCounter.containsKey(specificCaseExpression)
				? specificCaseExpressionsCounter.get(specificCaseExpression)
				: 0;
	 ...
	 * The main method.
	 * 
	 * @param args args[0] contains the language code, args[1] the path of the xml
	 *             dump, args[2] theh path of the output file
	 * @throws IOException
	 * @throws CompressorException
	 */
	public static void main(String[] args) throws IOException, CompressorException {
		...
		extractor.extractSpecificCaseExpressions(Languages.getLanguageForShortCode(args[0]), args[1], args[2]);
	}
}
