...
public class GermanSpellerRuleTest {
  ...
  public void testRuleWithGerman() throws Exception {
    final HunspellRule rule = new GermanSpellerRule(TestTools.getMessages("German"), Language.GERMANY_GERMAN);
  ...
  public void testRuleWithAustrianGerman() throws Exception {
    final HunspellRule rule = new GermanSpellerRule(TestTools.getMessages("German"), Language.AUSTRIAN_GERMAN);
    final JLanguageTool langTool = new JLanguageTool(Language.AUSTRIAN_GERMAN);
    commonGermanAsserts(rule, langTool);
    assertEquals(0, rule.match(langTool.getAnalyzedSentence("Der äußere Übeltäter.")).length);  // umlauts
    assertEquals(1, rule.match(langTool.getAnalyzedSentence("Der äussere Übeltäter.")).length);
  }
  ...
  public void testRuleWithSwissGerman() throws Exception {
    final HunspellRule rule = new GermanSpellerRule(TestTools.getMessages("German"), Language.SWISS_GERMAN);
    final JLanguageTool langTool = new JLanguageTool(Language.SWISS_GERMAN);
    commonGermanAsserts(rule, langTool);
