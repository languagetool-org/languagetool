...
do
  ...
  for url in `grep \<url\> ../../rules/$lang/grammar.xml | sed -r "s/.*<url>(.*)<\/url>/\1/g"`
  do
  ...
  echo $i of `grep \<url\> ../../rules/$lang/grammar.xml | wc -l` \<url\>s in $lang/grammar.xml are problematic
done
