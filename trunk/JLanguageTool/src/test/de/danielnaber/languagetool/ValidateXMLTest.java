...
public class ValidateXMLTest extends TestCase {
  ...
  public void test() {
    try {
      for (int i = 0; i < Language.LANGUAGES.length; i++) {
        ...
        validate("rules" + File.separator + lang.getShortName() + File.separator + "grammar.xml");
      }
    } catch (Exception e) {
      e.printStackTrace();
      fail();
  ...
  private void validate(String filename) throws SAXException, IOException, ParserConfigurationException {
    SAXParserFactory factory = SAXParserFactory.newInstance();
    factory.setValidating(true);
    SAXParser saxParser = factory.newSAXParser();
    String xml = StringTools.readFile(filename);
    final String decl = "<?xml version=\"1.0\" encoding=\"utf-8\"?>";
    final String dtd = "<!DOCTYPE rules PUBLIC \"-//W3C//DTD GrammarRules 0.1//EN\" \"file:rules/rules.dtd\">";
